...
TITOLARIO = {
    # [span_0](start_span)TITOLO I - AMMINISTRAZIONE GENERALE[span_0](end_span)
    ...
    # [span_1](start_span)TITOLO II - ORGANI DI GOVERNO, GESTIONE, CONTROLLO, CONSULENZA E GARANZIA[span_1](end_span)
    ...
    # [span_2](start_span)TITOLO III - RISORSE UMANE[span_2](end_span)
    ...
    # [span_3](start_span)TITOLO IV - RISORSE FINANZIARIE E PATRIMONIALI[span_3](end_span)
    ...
    # [span_4](start_span)TITOLO V - AFFARI GENERALI[span_4](end_span)
    ...
    # [span_5](start_span)TITOLO VI - PIANIFICAZIONE, GESTIONE DEL TERRITORIO[span_5](end_span)
    ...
    # [span_6](start_span)TITOLO VII - SERVIZI ALLA PERSONA[span_6](end_span)
    ...
    # [span_7](start_span)TITOLO VIII - ATTIVITA' ECONOMICHE[span_7](end_span)
    ...
    # [span_8](start_span)TITOLO IX - POLIZIA LOCALE E LA SICUREZZA PUBBLICA[span_8](end_span)
    ...
    # [span_9](start_span)TITOLO X - TUTELA DELLA SALUTE[span_9](end_span)
    ...
    # [span_10](start_span)TITOLO XI - SERVIZI DEMOGRAFICI[span_10](end_span)
    ...
    # [span_11](start_span)TITOLO XII - ELEZIONI, INIZIATIVE POPOLARI[span_11](end_span)
    ...
    # [span_12](start_span)TITOLO XIII - AFFARI MILITARI[span_12](end_span)
    ...
    # [span_13](start_span)TITOLO XIV - OGGETTI DIVERSI[span_13](end_span)
...
def create_folder_structure(client, bucket_name, folder_path):
    """Crea una struttura di cartelle in un bucket MinIO (simulata con un file .gitkeep)"""
    try:
...
def create_complete_structure(client):
    ...
    for bucket_name, classi in TITOLARIO.items():
        ...
        print(f"\n📁 {bucket_name_lower.upper()}")
        print("-" * 60)
        
        # 1. Crea il bucket principale (Titolo)
        create_bucket_if_not_exists(client, bucket_name_lower)
            
        # 2. [span_14](start_span)Gestione Speciale: Fascicoli del Personale (Solo per Titolo 03)[span_14](end_span)
        ...
        for classe_name, sottoclassi in classi.items():
            ...
            print(f"    ✓ Classe: {class_base_path}")
            ...
            for sottoclasse_name, sub_sottoclassi in sottoclassi.items():
                ...
                print(f"        -> Sottoclasse: {sottoclasse_path}")

                if sub_sottoclassi:
                    for sub_sottoclasse_name in sub_sottoclassi:
                        # Crea la cartella di Sub-Sottoclasse (Es. 2025/1.4-reg/1.4.1-reg/1.4.1.1-organigramma)
                        sub_sottoclasse_path = f"{sottoclasse_path}/{sub_sottoclasse_name}"
                        create_folder_structure(client, bucket_name_lower, sub_sottoclasse_path)
                        # Aggiungiamo le cartelle finali al livello più profondo
                        for final_subfolder in final_subfolders:
                            final_path = f"{sub_sottoclasse_path}/{final_subfolder}"
                            create_folder_structure(client, bucket_name_lower, final_path)
                        # Stampa la sottoclasse solo se ha ulteriori suddivisioni
                        print(f"            -> Sub-Sottoclasse: {sub_sottoclasse_path} (+ documentazione)")
                else:
                    # Se non ci sono ulteriori suddivisioni, crea qui le cartelle finali
                    for final_subfolder in final_subfolders:
                        final_path = f"{sottoclasse_path}/{final_subfolder}"
                        create_folder_structure(client, bucket_name_lower, final_path)
                    print(f"        -> Sottoclasse: {sottoclasse_path} (+ documentazione)")
...
def main():
    ...
    if not client:
        print("\n❌ Impossibile procedere senza connessione a MinIO")
        return
    ...
    print("\n" + "="*60)
    print("✅ STRUTTURA TITOLARIO COMPLETATA CON CLASSIFICAZIONE DETTAGLIATA")
    print("="*60)
